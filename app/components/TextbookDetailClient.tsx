                ...
                <ul className={`divide-y ${t.divideBorder} px-3 py-1.5`}>
                  {MATERIALS.map((m) => (
                    <li key={m.id} className="flex items-center gap-2 py-2">
                      <span className={`shrink-0 text-[10px] font-bold text-white px-1.5 py-0.5 rounded ${m.badge}`}>{m.type}</span>
                      <span className="text-xs font-semibold text-slate-700 flex-1">{m.label}</span>
                      <button
                        onClick={handleTeacherDownload}
                        className="ml-auto shrink-0 flex items-center gap-1 text-slate-500 hover:text-blue-600 bg-white border border-slate-200 hover:border-blue-300 transition-colors px-2 py-1 rounded-lg text-[10px] font-bold"
                      >
